...
`json`/`yaml`/`xml`/`ini`, большой текстовый файл, «битые» обрезки, файл без
расширения, архивы `zip`/`tar.gz`/`ar`/вложенный `zip`). Автор — этот
репозиторий, лицензия **CC0 1.0**. Воспроизводятся командой
