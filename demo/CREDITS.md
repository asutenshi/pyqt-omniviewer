...
`json`/`yaml`/`xml`/`ini`/`toml`, крупный `json` для проверки ленивого дерева,
минимальный `mid` (SMF, stdlib `struct`) и непрозрачный `bin` для fallback,
архивы `zip`/`tar.gz`/`ar`/вложенный `zip`, разметка
